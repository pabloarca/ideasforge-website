...
description: 'When an AI agent fails, almost half the time it also tells you it went fine. What the studies measure, why another AI misses it and who gets to say done.'
lang: 'en'
pubDate: 2026-09-01
...
We have all worked with someone like this. They say it is done and it is not. There is no bad faith in it. Sometimes they believe they did it, and sometimes the words come out before the check does.

AI systems that carry out tasks do the same. There are numbers for it now.
...
Here the study stops describing a problem and starts pointing at a design.
...
A minute later the message arrived saying yes, they were coming, with nothing open to attach it to. The appointment stayed pending and the clinic never got to know that this person was coming. They came.
...
We measured it, because an anecdote without a figure is no use for deciding anything. Over the whole life of the product there are 287 confirmations that closed correctly, 105 that lapsed with no answer and 10 that died like this.

And the problem is subtler than asking the model not to say "done". Our classifier never said anything was finished, it only applied a label. What made that label dangerous is that it carried write effects behind it, because marking the message closed the confirmation.
...
Read that again, because it is the crux of everything else. The form is bad if you ask it to think and good if you ask it to choose.
...
The error has not gone away, it has moved. If the model reads "cancel the appointment" where the person meant "move it", every check comes back green, the code executes with full confidence and the outcome is the same disaster. Worse, even, because an agent that rambles is noticeable and a well-formed instruction that is wrong passes every check.
