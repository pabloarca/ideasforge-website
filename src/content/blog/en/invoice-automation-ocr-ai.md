...
That last detail matters more than it might seem. **The administrative automation that survives does not arrive as one grand project that changes everything. It arrives process by process, starting with the one that hurts most.**
...
One day a utility redesigned its invoice without telling anyone, which is how these changes arrive. Since then the flow verifies the format before extracting anything. If an invoice does not match what is expected, it does not get processed. It stops and a person gets an alert with the document attached.
...
We learned this the expensive way. In one of our systems, a double timezone conversion shifted every entry time by two hours, without a single visible error. It was caught by comparing against the source document, not because anything failed. Since then the house rule is that **the silent error is the enemy**, because a badly extracted figure that looks well extracted propagates into every decision made with it. Doubtful cases go to a human review queue with a named owner. The rest pass on their own.
...
What survives all of this is the part that was the actual product. **A structured invoice saves you the OCR, not the validation.** That the totals add up, that the dates are possible and that anything odd stops matter just as much when the data arrives clean at source, because well-formed is not the same as correct.
...
The yardstick is still the one from the opening. *Can you trust what was read without looking at the paper?* The day the answer is yes, that process is off your desk.
