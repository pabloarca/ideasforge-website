---
title: 'Two kinds of "no" that are not the same: product limit or documentation gap'
...
The first is **"that cannot be done"**. A product policy: the system does not cover that use case. Asking a documentation assistant for real-time variables, or requesting an action nobody has wired up. The limitation is functional, and the correct answer exists before searching anything.

The second is **"that is not documented"**. An absence of evidence: the system searched and found nothing. The question was legitimate, the case is covered, but the documentation page that answers it is missing.
