...
It is half true. The missing half is the one carrying the obligations.

The Act does not sort companies by whether they write code. It sorts them by the role they play in relation to a given system. Whoever puts it on the market is the provider. Whoever runs it under their own authority is the deployer. **And the deployer has a list of obligations of their own**, which do not go away because somebody else trained the model.
...
Proportionate is the word that lets you breathe. It does not ask for a degree, it asks that whoever approves a generated answer knows what can go wrong inside it.

The second is the **transparency duty in Article 50**, binding since 2 August 2026. If a person is talking to a machine, they are told. If you publish a deepfake or AI-generated text on a matter of public interest, you say so.

Neither depends on your system being high risk. Neither has been postponed.

## The postponement almost everybody read inside out

In July 2026 the Digital Omnibus was published, Regulation (EU) 2026/1744. The reading that went around was **"they have delayed the AI Act"**. It went around so widely that plenty of boards filed the whole thing away.
...
Then Article 26 applies, which is the deployer's list. Use the system according to its instructions. Put human oversight in place with competence **and with the authority to stop the system**. Make sure the input data is relevant to what the system does, in so far as you control that data. Monitor how it behaves and report what goes wrong. Keep the logs for at least six months and tell the workers it affects. And tell the person the system decides about, when it is used to make a decision about them or to help make one.
...
If the answer is the second, it does not exist. A file assembled after the question is an account written after the fact, not evidence, and whoever reads it will be able to tell.
...
The full detail, with the eight domains one by one, Article 26 duty by duty and a first self-assessment you can run in an afternoon, is in [the EU AI Act guide](/en/eu-ai-act-compliance).
